...
// NestedConfig represents the configuration for nested grouping
type NestedConfig struct {
	Field     string         `json:"field"`             // Field to group by
	StructIn  string         `json:"struct_in"`         // Input struct name
	StructOut string         `json:"struct_out"`        // Output struct name for grouping
	Slice     *bool          `json:"slice,omitempty"`   // Whether to use slice (default: true)
	Pointer   *bool          `json:"pointer,omitempty"` // Whether to use pointers (default: true)
	Group     []NestedConfig `json:"group,omitempty"`
	Nested    []NestedConfig `json:"nested,omitempty"` // Nested structures within this one
}

// NestedQueryConfig represents nested grouping configuration for a specific query
type NestedQueryConfig struct {
	Query      string         `json:"query"`       // Query name to group
	Field      string         `json:"field"`       // Root field to group by
	StructRoot string         `json:"struct_root"` // Root struct name
	Group      []NestedConfig `json:"group"`       // Nested group configuration
}
...
type Options struct {
	...
	Nested                      []NestedQueryConfig `json:"nested,omitempty" yaml:"nested"`
